...
class InputData:
    ...
    def __init__(self, file_name, min_count=5):
        self.input_file_name = file_name
        self.get_words(min_count)
        self.word_pair_catch = deque()
        self.init_sample_table()
        print('Word Count: %d' % len(self.word2id))
        print('entence Length: %d' % (self.sentence_length))
    ...
    def get_words(self, min_count):
        ...
        #获取单词，统计词频放入临时变量word_frequency[单词] = 词频
        word_frequency = dict()
    ...
    def get_neg_v_neg_sampling(self, pos_word_pair, count):
        neg_v = numpy.random.choice(self.sample_table, size=(len(pos_word_pair), count))
        neg_v=neg_v.tolist()
        return neg_v
